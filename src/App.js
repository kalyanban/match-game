...
class App extends Component {
  ...
  imageClick = thumbnailUrl => {
    ...
    const imageVal = imagesList.filter(
      eachVal => eachVal.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = imageVal[0]
  ...
  render() {
    ...
    const thumbnailList = imagesList.filter(
      eachVal => eachVal.category === category,
    )
    return (
        ...
        <div className="content-container">
          {!isTrue && (
            <div className="first-div">
              <img src={imgUrl} className="display-image" alt="match" />
                  ...
                  <li key={eachItem.tabId}>
                    <button
                      ...
                      className={`tab-button ${
                        category === eachItem.tabId ? 'highlighted-text' : ''
                      }`}
                ...
                {thumbnailList.map(eachItem => (
                  <li key={eachItem.id}>
                    <button
                      ...
                      onClick={() => this.imageClick(eachItem.thumbnailUrl)}
                    >
                      <img
